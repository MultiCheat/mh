...
function main()
	...
	wait(1000)
	--[[
	WHSkeleton ñäåëàëè AppleThe & hnnssy
	Íåêîòîðûå ôóíêöèè âçÿòû èç ñîáåéòà cover
	FuckRadar By Trefa
	Íåêîòîðûå ôóíêöèè âçÿòû èç äðóãèõ ñîáåéòîâ.
	CamHack îò sanek a.k.a Maks_Fender, edited by ANIKI
	...
	downloadUrlToFile(update_url, update_path, function(id, status)
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then 
			...
			if tonumber(updateini.info.vers) > script_vers then
				sampAddChatMessage('Íàéäåíî Îáíîâëåíèå! âåðñèÿ:'..updateini.info.vers_text, -1)
				update_state = true
			end
	...
	sampRegisterChatCommand('/re', function(number)
		if number ~= '' then
			if number ~= '(%d+)' then
				...
				sampAddChatMessage('{FF0000}[Îøèáêà] {FF8C00}Óêàæèòå âðåìÿ ïåðåçàõîäà â ñåêóíäàõ.', 0xFFFF0000)
			end
		else
			sampAddChatMessage('{FF0000}[Îøèáêà] {FF8C00}Óêàæèòå âðåìÿ ïåðåçàõîäà.', 0xFFFF0000)
		end
	end)
	...
	while true do
		...
		imgui.Process = true
		if not MenuWindow.v and not DownMenu.v and not MenuWindow.v and not MenuCheats.v and not MenuVCheats.v and not MenuGun.v and not Other.v then
...
function imgui.OnDrawFrame()
	...
	if MenuWindow.v then
		...
		imgui.Begin(u8'Ìóëüòè-÷èò', MenuWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
		if imgui.Button(u8'×èòû', imgui.ImVec2(200, 50)) then
			MenuWindow.v = false
			MenuCheats.v = true
		end
		if imgui.Button(u8'Âðåäèòåëüñêèå ÷èòû', imgui.ImVec2(200, 50)) then
			MenuWindow.v = false
			MenuVCheats.v = true
		end
		if imgui.Button(u8'Îðóæèå', imgui.ImVec2(200, 50)) then
			MenuWindow.v = false
			MenuGun.v = true
		end
		...
		if imgui.Button(u8'Îñòàëüíîå', imgui.ImVec2(200, 50)) then
			MenuWindow.v = false
			Other.v = true
		end
		imgui.End()
	...
	if MenuCheats.v then
		...
		imgui.Begin(u8'×èòû', nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
		if imgui.Button(u8'Íàçàä <<', imgui.ImVec2(250, 30)) then
			MenuCheats.v = false
			MenuWindow.v = true
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] CBug Helper', elements.checkbox.CHelper) then
			HLcfg.config.CHelper = elements.checkbox.CHelper.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Áåñêîíå÷íûå ïàòðîíû', elements.checkbox.InfinityAmmo) then
			HLcfg.config.InfinityAmmo = elements.checkbox.InfinityAmmo.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] NoSpread', elements.checkbox.NoSpread) then
			HLcfg.config.NoSpread = elements.checkbox.NoSpread.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] AntiStun', elements.checkbox.AntiStun) then
			HLcfg.config.AntiStun = elements.checkbox.AntiStun.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Áåñêîíå÷íûé áåã', elements.checkbox.InifinityRun) then
			HLcfg.config.InifinityRun = elements.checkbox.InifinityRun.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àâòî +Ñ', elements.checkbox.AutoC) then
			HLcfg.config.AutoC = elements.checkbox.AutoC.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àèì', elements.checkbox.Aim) then
			HLcfg.config.Aim = elements.checkbox.Aim.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Bhop Mode', elements.checkbox.Bhop) then
			HLcfg.config.Bhop = elements.checkbox.Bhop.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Áåññìåðòèå', elements.checkbox.Inv) then
			HLcfg.config.Inv = elements.checkbox.Inv.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Êîëëèçèþ îáúåêòîâ', elements.checkbox.CollizionOb) then
			HLcfg.config.CollizionOb = elements.checkbox.CollizionOb.v
			save()
		end
		if imgui.Checkbox(u8'[+/-] Ñêîðîñòü àíèìàöèé', elements.checkbox.SetSpeedAnim) then
			HLcfg.config.SetSpeedAnim = elements.checkbox.SetSpeedAnim.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àíòè-Àôê', elements.checkbox.AntiAfk) then
			HLcfg.config.AntiAfk = elements.checkbox.AntiAfk.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] NameTag', elements.checkbox.NameTag) then
			HLcfg.config.NameTag = elements.checkbox.NameTag.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] NoFall', elements.checkbox.NoFall) then
			HLcfg.config.NoFall = elements.checkbox.NoFall.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Áåñêîíå÷íûé êèñëîðîä', elements.checkbox.InfinityOxygen) then
			HLcfg.config.InfinityOxygen = elements.checkbox.InfinityOxygen.v 
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] NoBike', elements.checkbox.NoBike) then
			HLcfg.config.NoBike = elements.checkbox.NoBike.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Îãíåóïîðíîñòü', elements.checkbox.FireResistance) then
			HLcfg.config.FireResistance = elements.checkbox.FireResistance.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Åçäó áåç ïðàâ', elements.checkbox.DriveNoLicenze) then
			HLcfg.config.DriveNoLicenze = elements.checkbox.DriveNoLicenze.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] TriggerBot', elements.checkbox.AutoShot) then
			HLcfg.config.AutoShot = elements.checkbox.AutoShot.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] High jump on BMX', elements.checkbox.JBMX) then
			HLcfg.config.JBMX = elements.checkbox.JBMX.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Áåñêîíå÷íîå çäîðîâüå', elements.checkbox.InfinityHealth) then
			HLcfg.config.InfinityHealth = elements.checkbox.InfinityHealth.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Ìåãà Ïðûæîê', elements.checkbox.MJump) then
			HLcfg.config.MJump = elements.checkbox.MJump.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àíòè-Áëîê Äâèãàòåëÿ', elements.checkbox.AntiBlockCar) then
			HLcfg.config.AntiBlockCar = elements.checkbox.AntiBlockCar.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àíòè-Çàïðàâêà', elements.checkbox.AntiZP) then
			HLcfg.config.AntiZP = elements.checkbox.AntiZP.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Åçäó ïî âîäå', elements.checkbox.DriveInWater) then
			HLcfg.config.DriveInWater = elements.checkbox.DriveInWater.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àíòè-âçðûâ ïðè ïåðåâîðîòå', elements.checkbox.AntiBoomPDownCar) then
			HLcfg.config.AntiBoomPDownCar = elements.checkbox.AntiBoomPDownCar.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] ÂÕ Ñêåëåò', elements.checkbox.WHSkeleton) then
			HLcfg.config.WHSkeleton = elements.checkbox.WHSkeleton.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] ÂÕ Êëàäîâ [ARZ]', elements.checkbox.ForClad) then
			HLcfg.config.ForClad = elements.checkbox.ForClad.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] ÂÊ Çàêëàäîê [ARZ]', elements.checkbox.NarkoWH) then
			HLcfg.config.NarkoWH = elements.checkbox.NarkoWH.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Íåâèäèìêó', elements.checkbox.Ghost) then
			HLcfg.config.Ghost = elements.checkbox.Ghost.v
			save()
		end
		imgui.End()
	...
	if MenuVCheats.v then
		...
		imgui.Begin(u8'Âðåä.×èòû', nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
		if imgui.Button(u8'Íàçàä <<', imgui.ImVec2(200, 30)) then
			MenuVCheats.v = false
			MenuWindow.v = true
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] CarShot', elements.checkbox.CarShot) then
			HLcfg.config.CarShot = elements.checkbox.CarShot.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Ðâàíêó', elements.checkbox.Rvanka) then
			HLcfg.config.Rvanka = elements.checkbox.Rvanka.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Ëàãè', elements.checkbox.Lugs) then
			HLcfg.config.Lugs = elements.checkbox.Lugs.v
			save()
		end
		imgui.End()
	...
	if MenuGun.v then
		...
		imgui.Begin(u8'Âûäàòü îðóæèå', nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
		if imgui.Button(u8'Íàçàä <<', imgui.ImVec2(200, 30)) then
			MenuGun.v = false
			MenuWindow.v = true
		end
		imgui.Text(u8'Ââåäèòå êîë-âî ïàòðîí')
		imgui.InputText(u8'##GiveGun', AmmoGun)
		imgui.Text(u8'Âûáåðèòå îðóæèå')
		imgui.Combo('##COMBO', selectGun, ComboGun)
		if imgui.Button(u8'Âûäàòü', imgui.ImVec2(200, 0)) then
			if AmmoGun.v ~= '' then
				checks()
			else
				sampAddChatMessage('{FF0000}[Îøèáêà] {FF8C00}Óêàæèòå êîë-âî ïàòðîí.', 0xFFFF0000)
			end
		end
		imgui.End()
	end
	if Other.v then
		...
		imgui.Begin(u8'Ïðî÷åå', nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
		if imgui.Button(u8'Íàçàä <<', imgui.ImVec2(250, 30)) then
			Other.v = false
			MenuWindow.v = true
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] ×èñòûé ñêðèí', elements.checkbox.SafeScreen) then
			HLcfg.config.SafeScreen = elements.checkbox.SafeScreen.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] ×àò íà T', elements.checkbox.ChatT) then
			HLcfg.config.ChatT = elements.checkbox.ChatT
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Ñåðûé ðàäàð', elements.checkbox.GRadar) then
			HLcfg.config.GRadar = elements.checkbox.GRadar.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] AntiBlockedPlayer', elements.checkbox.BlockedPlayer) then
			HLcfg.config.BlockedPlayer = elements.checkbox.BlockedPlayer.v 
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Î÷èñòêó /dl', elements.checkbox.ClearDl) then
			HLcfg.config.ClearDl = elements.checkbox.ClearDl.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Áûñòðóþ îñòàíîâêó', elements.checkbox.MStop) then
			HLcfg.config.MStop = elements.checkbox.MStop.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] AntiCarSkill', elements.checkbox.AntiCarSkill) then
			HLcfg.config.AntiCarSkill = elements.checkbox.AntiCarSkill.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] AntiEject', elements.checkbox.AntiEject) then
			HLcfg.config.AntiEject = elements.checkbox.AntiEject.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] NoRadio', elements.checkbox.NoRadio) then
			HLcfg.config.NoRadio = elements.checkbox.NoRadio.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Ôóíêöèþ "Âñåãäà ñûòûé"', elements.checkbox.NoEatAll) then
			HLcfg.config.NoEatAll = elements.checkbox.NoEatAll.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Òðåéñåð ïóëü', elements.checkbox.Traicer) then
			HLcfg.config.Traicer = elements.checkbox.Traicer.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àíòè-Ðâàíêà', elements.checkbox.AntiRv) then
			HLcfg.config.AntiRv = elements.checkbox.AntiRv.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Àíòè-Ðåêîí', elements.checkbox.AntiRecon) then
			HLcfg.config.AntiRecon = elements.checkbox.AntiRecon.v
			save()
		end
		imgui.End()
	...
	if MenuPatch.v then
		...
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop NoFreeze', elements.checkbox.NoFreeze) then
			HLcfg.config.NoFreeze = elements.checkbox.NoFreeze.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop ShowDialog', elements.checkbox.ShowDialog) then
			HLcfg.config.ShowDialog = elements.checkbox.ShowDialog.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop Spectate', elements.checkbox.Spectate) then
			HLcfg.config.Spectate = elements.checkbox.Spectate.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop Health', elements.checkbox.NHealth) then
			HLcfg.config.NHealth = elements.checkbox.NHealth.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop GiveGun', elements.checkbox.GiveGun) then
			HLcfg.config.GiveGun = elements.checkbox.GiveGun.v 
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop ResetGun', elements.checkbox.ResetGun) then
			HLcfg.config.ResetGun = elements.checkbox.ResetGun.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop SetGun', elements.checkbox.SetGun) then
			HLcfg.config.SetGun = elements.checkbox.SetGun.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop RequestClass', elements.checkbox.RequestClass) then
			HLcfg.config.RequestClass = elements.checkbox.RequestClass.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop RequestSpawn', elements.checkbox.RequestSpawn) then
			HLcfg.config.RequestSpawn = elements.checkbox.RequestSpawn.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop ApplyAnimation', elements.checkbox.ApplyAnimation) then
			HLcfg.config.ApplyAnimation = elements.checkbox.ApplyAnimation.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop ClearAnimation', elements.checkbox.ClearAnimation) then
			HLcfg.config.ClearAnimation = elements.checkbox.ClearAnimation.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop ForceClass', elements.checkbox.ForceClass) then
			HLcfg.config.ForceClass = elements.checkbox.ForceClass.v
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop FacingAngle', elements.checkbox.FacingAngle) then
			HLcfg.config.FacingAngle = elements.checkbox.FacingAngle.v 
			save()
		end
		if imgui.Checkbox(u8'[Âêë/âûêë] Nop ToggleControl', elements.checkbox.ToggleControl) then
			HLcfg.config.ToggleControl = elements.checkbox.ToggleControl.v
			save()
		end
		imgui.End()
		...
		if elements.checkbox.NarkoWH.v then
			for n = 1, 2048 do
				if sampIs3dTextDefined(n) then
					...
					if isPointOnScreen(posX, posY, posZ, 0.0) and string.find(string, 'Çàêëàäêà') then
			...
			for _, objid in pairs(getAllObjects()) do
				if tonumber(getObjectModel(objid)) == 1271 or tonumber(getObjectModel(objid)) == 2680 then
					...
					if isObjectOnScreen(objid) then
						...
						renderFontDrawText(font, "{3c8fb5}[WH] {ffffff} Êëàä", wposX + 5, wposY - 7, 0xcac1f4c1)
					end
				end
			end
...
function samp.onCreate3DText(id,color,position,dist,testLOS,attachedPlayerId,attachedVehicleId,text)
    if color == -1 and text:find("Ðàäàð%s*Ìàêñèìàëüíàÿ ñêîðîñòü: {D1DE43}%d+ êì/÷.{FFFFFF}%s*Øòðàô çà íàðóøåíèå: {D1DE43}%d+%$.{FFFFFF}%s*Ïîñòàâèë: {D1DE43}.*") then
